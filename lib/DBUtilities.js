...
class DBUtilities {
   ...
   * @returns {Promise<*>}
   */
  createInDB = async (table, data, addTimeStamps = true, addOriginalDataCopy = true) => {
    try {
      if (addTimeStamps === true) {
        data = { ...data, 'Created': new Date().toISOString() };
      }
      if (addOriginalDataCopy === true) {
        data = { ...data, 'OriginalObject': data }; // todo may be able to use ADD instead of SET so that forceUpdate using import script doesn't wipe this original object out
      }
  ...
  updateInDB = async (table, data, patch = false, addTimeStamps = true) => {
    try {
      ...
      } else { // todo may be better to have this as default
        let params = {
          TableName: table,
          Key: data.Key
        };
  ...
  deleteInDB = async (table, key, value, soft = false, addTimeStamps = true) => {
    try {
      ...
      if (soft === true) { // todo may be better to have this as default
   ...
   * @returns {Promise<*>}
   */
  recoverItem = async (table, key, addTimeStamps = true) => {
    try {
      let databaseResult;
      ...
      if (addTimeStamps) {
        ...
        updateExpression = {
          TableName: table,
          Key: key,
          UpdateExpression: "SET #deleted = :false, #Updated = :updated",
          ExpressionAttributeNames: {
            "#deleted": "deleted",
            "#Updated": "Updated",
          },
      ...
      await this._errorHandler.handleError('recoverItem', 'Error recovering from Database', error);
      throw new AWSDBError(error.message);
    }
  }
