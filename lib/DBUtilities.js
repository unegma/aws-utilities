...
class DBUtilities {
   ...
   * @returns {Promise<*>}
   */
  getFromDB = async (table, keys, index = null, values) => {
    try {
      let databaseResult;
      if (!index) {
        let keyValue = {}
        ...
        if (Array.isArray(keys) && Array.isArray(values)) {
          keys.forEach((k, i) => keyValue[k] = values[i]);
        } else {
   ...
   * @returns {Promise<*>}
   */
  updateInDB = async (table, data, patch = false) => {
    try {
      let databaseResult;
      if (patch === false) {
   ...
   * Delete in Database
   *
   * key can be a single value or an array as can values (see test)
   * make sure to pass the correct primary key as part of the data
   *
   * @param table
   * @param keys
   ...
   * @returns {Promise<*>}
   */
  deleteInDB = async (table, keys, values, soft = false) => {
    try {
      let keyValue = {};
      let databaseResult;
