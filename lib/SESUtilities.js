...
class SESUtilities {
   ...
   * @returns {Promise<*>}
   */
  sendEmail = async (to, from, subject, body, template = null) => {
    try {
      let params = {
        Content: {
          Simple: {
      ...
      if(template) {
        params.Content['Template'] = template; // TemplateArn TemplateData TemplateName
      }

      let sesResult = await ses.sendEmail(params).promise();
